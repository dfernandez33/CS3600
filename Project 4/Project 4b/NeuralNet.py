...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for weight in self.weights:
            newWeight = weight + (alpha * inActs[counter] * delta)
            newWeights.append(newWeight)
            totalModification += abs(newWeight - weight)
            counter += 1
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            for neuron in layer:
                if len(prevInputs) == 0:
                    currentInputs.append((neuron.sigmoidActivation(inActs)))
                else:
                    currentInputs.append(neuron.sigmoidActivation(prevInputs))
            prevInputs = currentInputs
            layerOutputs.append(currentInputs)
        return layerOutputs
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in xrange(len(example[1])):
                gPrime = self.outputLayer[outputNum].sigmoidActivationDeriv(allLayerOutput[1])
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for neuron in nextLayer:
                        error += neuron.weights[neuronNum + 1] * deltas[0][nextLayer.index(neuron)]
                    delta = gPrime *  error
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for counter in range(len(examplesTest)):
        currentTest = examplesTest[counter]
        outputs = nnet.feedForward(currentTest[0])
        lastIndex = len(outputs) - 1
        for index in range(len(outputs[lastIndex])):
            outputs[lastIndex][index] = round(outputs[lastIndex][index])
        if not outputs[lastIndex] == currentTest[1]:
            testError += 1
        else:
            testCorrect += 1
    
    testAccuracy= float(testCorrect) / float(len(examplesTest)) #must cast to float in order to avoid integer division
