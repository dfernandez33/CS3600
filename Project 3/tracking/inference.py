    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newp, probability in newDistribution.items():
                newBeliefs[newp] += self.beliefs[p] * probability
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for position in self.legalPositions:
            for x in range(particlesPerPosition):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for particle in self.particles:
                totalDistance = util.manhattanDistance(pacmanPosition, particle)
                newBeliefs[particle] += emissionModel[totalDistance]
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            if particle is not previousParticle:
                newDistribution = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
            newParicles.append(util.sample(newDistribution))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        ghosts = itertools.product(self.legalPositions, repeat=self.numGhosts)
        ghostList = list(ghosts)
        random.shuffle(ghostList)

        particalesPerPosition = self.numParticles / len(ghostList)
        newParticles = []

        for x in range(particalesPerPosition):
            newParticles = newParticles + ghostList
        self.particles = newParticles
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for p in self.particles:
            ...
            for x in range(self.numGhosts):
                currentNoisyDistance = noisyDistances[x]
                currentEmissionModel = emissionModels[x]
                if currentNoisyDistance is None:
                    p = self.getParticleWithGhostInJail(p, x)
                else:
                    trueDistance = util.manhattanDistance(pacmanPosition, p[x])
                    probability *= currentEmissionModel[trueDistance]
            newBeliefs[p] += probability
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for x in range(self.numGhosts):
                sample = util.sample(
                    getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), x, self.ghostAgents[x])
                )
                newParticle[x] = sample
