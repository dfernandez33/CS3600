...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        nearestDist = 10000000
        nearestSquare = None
        for ghostDistribution in livingGhostPositionDistributions:
            probableState = max(ghostDistribution, key=ghostDistribution.get)
            distanceToGhost = self.distancer.getDistance(pacmanPosition, probableState)
            if distanceToGhost < nearestDist:
                nearestDist = distanceToGhost
                nearestSquare = probableState

        bestAction = None
        bestNewDistance = 10000000
        for action in legal:
            newDistance = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), nearestSquare)
